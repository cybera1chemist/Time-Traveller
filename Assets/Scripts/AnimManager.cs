...
public class AnimManager : MonoBehaviour
{
    ...
    {
        // 如果看过这段动画剧情了，就不放了
        if (PlayerPrefs.GetInt($"Anim_{animID}_Completed", 0) == 1)
        {
            TryLoadMainMenu();
            return;
        }
    ...
    void Update()
    {
        ...
        {
            Debug.Log("[AnimManager] Input detected in Update(). isTyping = " + isTyping);
    ...
    void TryLoadMainMenu()
    {
        ...
        if (index != -1)
        {
            PlayerPrefs.SetInt($"Anim_{animID}_Completed", 1);
            PlayerPrefs.Save();
            SceneManager.LoadScene(index);
            return;
        } else
        {
            Debug.LogError("[AnimManager] Cannot load MainMenu scene! Check the scene path in AnimManager.");
        }
    }
}
